...
$allowed_counter_ip = array('118.98.234.40');
...
if (isset($_POST['counter'])) {
  ...
  */
  function setCounter($str_member_ID) {
    ...
    // if member is already registere
    if ($_q->num_rows > 0) {
        $_d = $_q->fetch_assoc();
        if ($_d['is_expire'] == 1) {
            $expire = 1;
  ...
  if ($counter === true) {
    echo __($member_name.', Terima Kasih atas kunjungannya Di Perpustakaan Terpadu Ditjen Kebudayaan').'<div id="memberImage" data-img="./images/persons/'.urlencode($photo).'"></div>';
    if ($expire) {
