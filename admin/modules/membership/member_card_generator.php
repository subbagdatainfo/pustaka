...
if (isset($_GET['action']) AND $_GET['action'] == 'print') {
	...
	*/
    ...
    $html_str .= '.bio_div{z-index:7;position: absolute;left: 0px;font-size: 9px;top:53px;height: 110px;margin: 0px;text-align: justify;}'."\n";
    ...
    $html_str .= '.label_address{z-index:10;float: left; width: 200px;margin-bottom:px;margin-left:3px;}'."\n";
    ...
    $html_str .= '.city{z-index:13;font-size:9px;margin-left : -12px;top:-10px}'."\n";
    $html_str .= '.title{z-index:14;font-size:9px;px;margin-left:-12px;top:-10px}'."\n";
    $html_str .= '.officials{z-index:15;top: 5px;font-size: 9px;margin-left:-12px;top:-6px}'."\n";
    $html_str .= '.sign_file_div{z-index:16;position: absolute;left: 0px;top: -13px;width:400px;height: 25px;}'."\n";
    $html_str .= '.stamp_file_div{z-index:17;position: absolute;left:50px;top: -13px;width: 100px;height: 100px;}'."\n";
    $html_str .= '.exp_div{z-index:18;position: absolute;right : 22px;top: 135px;width:100px;height: 12px;font-size: 7px;text-align: right;}'."\n";
    $html_str .= '.barcode_div{z-index:19;position: absolute;right : 22px;top: 150px;width: 100px;height: 60px;}'."\n";
    $html_str .= '</style>'."\n";
    $html_str .= '</head>'."\n";
    $html_str .= '<body>'."\n";
    ...
    foreach ($chunked_card_arrays as $membercard_rows) {
        ...
        foreach ($membercard_rows as $card) {
          ...
          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<p class="bio_address"><label class="bio_label">'.__('Address').' / '.__('No Telp').'</label><span style="float:left">: </span>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';
          $html_str .= ''.( $sysconf['print']['membercard']['include_address_label']?'':'<!--').'<span class="label_address">'.$card['member_address'].' / '.$card['Phone Number'].'</span></p>'.( $sysconf['print']['membercard']['include_address_label']?'':'-->').'';
          $html_str .= '</div>';
          ...
          $html_str .= '<img  width="250px" height="40px" src="'.SWB.IMG.'/barcodes/'.str_replace(array(' '), '_', $card['member_id']).'.png" style="width:'.$sysconf['print']['membercard']['barcode_scale'].'%; border="10px" /></img></div>'.( $sysconf['print']['membercard']['include_barcode_label']?'':'-->').'';
          $html_str .= '<div class="stamp_div">';
          $html_str .= '<div class="stamp_file_div"></div>';
          $html_str .= '<div class="sign_file_div"></div>';
          $html_str .= '<p class="stamp city">'.$sysconf['print']['membercard']['city'].', '.$card['register_date'].'</p><p class="stamp title">'.$sysconf['print']['membercard']['title'].'</p>';
          $html_str .= '<p class="stamp officials">'.$sysconf['print']['membercard']['officials'].''.$sysconf['print']['membercard']['officials_id'].'</p></div></div></td>';
          $html_str .= '<td valign="top">';
